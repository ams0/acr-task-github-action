...
if [ "$INPUT_GIT_ACCESS_TOKEN" != "" ]; then
...
az acr build $GIT_ACCESS_TOKEN_FLAG -r ${INPUT_REGISTRY} -f ${INPUT_DOCKERFILE} -t ${INPUT_REPOSITORY}${IMAGE_PART}:${INPUT_TAG} https://github.com/${GITHUB_REPOSITORY}.git#${INPUT_BRANCH}:${INPUT_FOLDER}